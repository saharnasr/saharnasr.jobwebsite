...
namespace WebApplication1.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ViewBag.UserType = new SelectList(db.Roles.Where(a => !a.Name.Contains("Adminstrators")).ToList(), "Name", "Name");
        ...
        [HttpPost]
        public ActionResult EditProfile(EditProfileViewModel profie)
        {
            var UserID = User.Identity.GetUserId();
            var CurrentUser = db.Users.Where(a=>a.Id==UserID).SingleOrDefault();
            if (!UserManager.CheckPassword(CurrentUser,profie.CurrentPassword))
            {
            ...
            else
            {
                ...
                var newPasswordHash = UserManager.PasswordHasher.HashPassword(profie.NewPassword);
                CurrentUser.UserName = profie.UserName;
                CurrentUser.Email = profie.Email;
